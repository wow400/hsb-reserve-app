...
async function handleStatus(request, env) {
  const url = new URL(request.url);
  if (!env.AVIATIONSTACK_KEY) return jsonResponse({ ok: false, error: "Missing AVIATIONSTACK_KEY secret" }, 500);
  ...
  for (const flight of requestedFlights) {
    ...
    let best = null, bestVariant = null, lastError = null;
    ...
    for (const variant of variants) {
      ...
      try {
        ...
        if (apiJson && apiJson.error) { lastError = apiJson.error; continue; }
        const data = Array.isArray(apiJson.data) ? apiJson.data : [];
        const match = pickBestMatch(data, variant);
        if (match) { best = match; bestVariant = variant; break; }
      } catch (err) {
    ...
    if (!best) {
      results[flight] = { ...emptyFlight(flight), error: lastError || null, tried: variants };
      continue;
    }
    ...
    results[flight] = {
      ...
      departure: {
        airport: dep.airport || null, iata: dep.iata || null, timezone: dep.timezone || null,
        terminal: dep.terminal || null, gate: dep.gate || null, delay: dep.delay ?? null,
        scheduled: dep.scheduled || null, estimated: dep.estimated || null, actual: dep.actual || null,
        estimated_runway: dep.estimated_runway || null, actual_runway: dep.actual_runway || null
      },
      arrival: {
        airport: arr.airport || null, iata: arr.iata || null, timezone: arr.timezone || null,
        terminal: arr.terminal || null, gate: arr.gate || null, baggage: arr.baggage || null,
        delay: arr.delay ?? null, scheduled: arr.scheduled || null, estimated: arr.estimated || null,
        actual: arr.actual || null, estimated_runway: arr.estimated_runway || null, actual_runway: arr.actual_runway || null
      },
...
function emptyFlight(flight) {
  return { flight, lookup_flight: null, found: false, status: "unknown", label: "Unknown", confidence: "none", safe_by_status: false, raw_status: null, flight_date: null, departure: null, arrival: null, aircraft: null, live: null };
}
...
function jsonResponse(body, status = 200) {
  return new Response(JSON.stringify(body, null, 2), { status, headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*", "Access-Control-Allow-Methods": "GET, OPTIONS", "Access-Control-Allow-Headers": "Content-Type" } });
}
...
<style>
:root{--bg:#f3f4f6;--card:#fff;--ink:#111;--muted:#555;--line:#e1e4e8;--soft:#f0f3f7;--ok:#0a6b28;--amber:#8a5a00;--red:#b00020;--call-bg:#fff4ce;--safe-bg:#e9f7ec;--live-bg:#eef8f0;--crit-bg:#fff0f0;--pre-bg:#eef3ff;--dep-bg:#f1f1f1;--bad:#a40000}
...
.next-title{font-weight:900;margin-bottom:5px}.next-strong{font-weight:900}
...
function dur(mins){mins=Math.abs(mins);return Math.floor(mins/60)+"h"+String(mins%60).padStart(2,"0")+"m"}
...
function computeRows(){const startInput=document.getElementById("hsbStart").value,endInput=document.getElementById("hsbEnd").value;if(!startInput||!endInput)return null;const hsbStart=toMin(startInput),hsbEnd=normaliseEnd(hsbStart,toMin(endInput)),latestOnBlocks=hsbStart+HSB_TO_CHOCKS_LIMIT,now=utcNowMinutes();const hsbStartDelta=futureDelta(hsbStart,now),hsbFinishDelta=futureDelta(hsbEnd,now);const hsbNotStarted=hsbStartDelta>0&&hsbStartDelta<720,hsbFinished=hsbFinishDelta<0;const rows=flights.map(f=>{const latestTO=latestOnBlocks-f.block,latestCall=latestTO-CALL_BEFORE_TAKEOFF,safeAfter=Math.min(latestCall,hsbEnd),delta=futureDelta(safeAfter,now),safeReason=hsbEnd<latestCall?"HSB finish":"Latest call",fs=statuses[f.flight]||{status:"unknown",found:false,label:"Unknown",confidence:"none",safe_by_status:false};return{...f,latestOnBlocks,latestTO,latestCall,safeAfter,delta,safeReason,fs}});return{rows,hsbStart,hsbEnd,latestOnBlocks,now,hsbStartDelta,hsbFinishDelta,hsbNotStarted,hsbFinished}}
